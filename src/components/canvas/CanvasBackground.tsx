...
// Skalierung/Position folgen dem Pan/Zoom des InfiniteCanvas (canvasView aus
// uiStore, von InfiniteCanvas bei jedem Wheel-/Pointer-Event aktualisiert) —
// dieselbe translate()+scale()-Logik wie beim eigentlichen Widget-Inhalt,
// nur über backgroundPosition/backgroundSize statt einem CSS-Transform, weil
// dieser Layer bewusst außerhalb des transformierten Containers liegt (s.
// BoardGrid.tsx). Punktgröße/Linienstärke bleiben bewusst fix (wie bei den
// meisten Infinite-Canvas-Tools) — nur der ABSTAND soll mit reinzoomen.
function patternCss(bg: BoardBg, view: { x: number; y: number; zoom: number }): React.CSSProperties {
  const pos = `${view.x}px ${view.y}px`
  if (bg.pattern === 'dots') {
    const size = 28 * view.zoom
    return {
